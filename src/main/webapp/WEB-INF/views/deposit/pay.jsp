...
<div class="bodyer pay notPay">
	<div class="page">
    	<form name="pay" method="post" action="http://www.17xs.org/alipay/deposit.do" method="post">
    	<!-- <div class="page-hd">
        ...
        <div class="page-bd"> 
            <div class="pay-hd">
				<div class="pay-tit">
            		<h3>认捐主题：${pName}</h3>
                    <ul class="pay-data">
                        <li class="pay-price">单价：<fmt:formatNumber value="${permoney}" pattern="0.00"/>元</li>
                        <li class="pay-count" title="最多可认捐${leaveCopies}份">认捐<span><i id="btn-reduce">-</i><input id="num" name="payCount" type="text" value="${copies}"/><i id="btn-add">+</i></span>份</li>
                        <c:if test="${special_fund_id==0 || special_fund_id==null }">
                        <li class="pay-money"><span>需支付：<em  class="b-color"><i id="payMoney"><fmt:formatNumber value="${amount}" pattern="0.00"/></i>元</em></span></li> 
                    	</c:if>
                    </ul>
                </div>
                <div class="pay-info">
                	<p class="pay-promot"><i class="promot-icon"></i>请填写您的姓名 / 公司名称及募捐赠言（可选填），以便善园对您的认捐的项目进行实体镌刻。</p>
                    <div class="pay-msg">
                      <label><i class="guide-icon"></i>姓名/公司名称</label><input id="donateName" name="donateName" maxlength="20" type="text" value="" />
                      <label><i class="guide-icon"></i>赠言</label><input id="donateWord" name="donateWord" maxlength="20" type="text" value=""  placeholder="请控制在20个字以内"/>
                    </div>
                </div> 
            </div>
            <div class="page-entBd">
                <div class="pay-hdm">
                    <h2>支付确认</h2>
                    <ul class="pay-tit">
                        <li>助善主题：${pName}</li>
                        <li><label for="isUsable"><input type="checkbox" id="isUsable" checked=""> 可用余额：<fmt:formatNumber value="${balance}" pattern="0.00"/>元</label><span class="more">剩余支付：<strong><span id="stillMoney"><fmt:formatNumber value="${copies*permoney - balance}" pattern="0.00"/></span>元</strong></span></li>
                    </ul>
                </div>
            </div>
    	</div>
        ...
        <div class="pageCon yh" id="chargePay" style="display:none">
    ...
    <span>开户行：招商银行宁波分行鄞州支行</span>
    	...
    	<input type = "hidden"  id="permoney" name="permoney" value="${permoney}"/>
    	<input type = "hidden"  id="leaveCopies" name="leaveCopies" value="${leaveCopies}"/>
    	<input type = "hidden"  id="copies" name="copies" value="${copies}"/>
...
<script data-main="<%=resource_url%>res/js/dev/payShanyuan.js" src="<%=resource_url%>res/js/require.min.js"></script>
</body>
</html>
