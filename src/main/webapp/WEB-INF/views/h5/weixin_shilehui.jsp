...
<link rel="stylesheet" type="text/css"
	href="<%=resource_url%>res/css/h5/wenxin_2.css" />
</head>
		...
		function pay() {
			wx.chooseWXPay({
				...
				success : function(res) {
					var url = "http://m.shilehui.com";
					window.location.href = url;
				}
			});
		}
		function alipay(){
			var url = "http://www.17xs.org/WapAlipay/deposit.do?projectId="+$('#projectId').val()+"&amount="
						+$('#amount').val()+"&extensionPeople="+$('#extensionPeople').val()
							+"&payType=5" +"&copies="+$("#copies").val()+"&donateWord="+$("#donateWord").val()+"&crowdFunding="+$("#crowdFunding").val()+"&t="+ new Date().getTime();
			window.location.href = url;
		}
