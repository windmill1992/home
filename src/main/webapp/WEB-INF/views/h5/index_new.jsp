<!DOCTYPE html>
<html lang="zh-cmn-Hans">
<head>
	<meta charset="utf-8" />
	<meta name="viewport" content="width=device-width, user-scalable=0, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
	<%@ page contentType="text/html;charset=UTF-8" language="java" %>
	<%@ include file="./../common/file_url.jsp" %>
	<meta name="keywords" content="善园网，扶贫济困，公益援助，求助，募捐，医疗救助，教育救助，贫困救助，灾害救助，公益众筹，特殊群体" />
	<meta name="description" content="善园网，依托善园公益基金会开展网络募捐服务，通过互联网实施扶贫济困、慈善救助、公益援助等领域的救助行动；善园网承诺善款100%公开，100%用于受助人" />
	<meta name="viewport" />
	<title>善园 - 一起行善，温暖前行！</title>
	<link rel="stylesheet" type="text/css" href="../../../../res/css/dev/base.css" />
	<link rel="stylesheet" type="text/css" href="../../../../res/css/dev/dialog.css" />
	<link rel="stylesheet" type="text/css" href="../../../../res/css/dev/head.css" />
	<link rel="stylesheet" type="text/css" href="../../../../res/css/dev/form.css" />
	<link rel="stylesheet" type="text/css" href="../../../../res/css/h5/home_new.css" />
	<link rel="stylesheet" type="text/css" href="../../../../res/css/h5/home_xin.css" />
	<script type='text/javascript'>
		window.BWEUM || (BWEUM = {});
		BWEUM.info = {
			"stand": true,
			"agentType": "browser",
			"agent": "bi-collector.oneapm.com/static/js/bw-send-411.4.5.js",
			"beaconUrl": "bi-collector.oneapm.com/beacon",
			"licenseKey": "I66F7~vv2ai6V3Nn",
			"applicationID": 2282681
		};
	</script>
	<script type="text/javascript" src="//bi-collector.oneapm.com/static/js/bw-loader-411.4.5.js"></script>
	<style type="text/css">
		@media only screen and (min-width: 320px) and (max-width: 640px) {
			.scroll_box {max-height: 100px;}
		}
	</style>
</head>

<body>
	<div id="pageContainer">
		<div class="home">
			<div class="site_info">
				<span class="logo">
	  				<img src="../../../../res/images/h5/images/logo-lg.jpg" width="110%">
	  			</span>
				<a href="../../../../ucenter/userCenter_h5.do" class="ucen"><img src="../../../../res/images/h5/images/usercen.png" width="20" alt="用户中心" /></a>
				<span class="txt" id="total">捐助<span class="num"></span>元</span>
			</div>
			<div style="clear: both;"></div>
			<!--焦点图-->
			<!--scroll-->
			<div class="scroll relative">
				<div class="scroll_box" id="scroll_img">
					<ul class="scroll_wrap">
						<c:forEach items="${bannerList}" var="img">
							<li><a href="${img.linkUrl}"><img src="${img.url}" width="100%" /></a></li>
						</c:forEach>
					</ul>
				</div>
				
			</div>
			<!--scroll-->
			<!--分类//-->
			<div class="prsee_xin">
				<ul>
					<li class="pxin_l">
						<a href="http://www.17xs.org/project/batch_list.do?extensionPeople=${extensionPeople }" class="pxin_a">
							<span class="pxin_s pxin_s1"></span>
							<p class="pxin_p1">批量捐</p>
						</a>
					</li>
					<li class="pxin_l">
						<a href="../../../../user/getDonateTime.do" class="pxin_a">
							<span class="pxin_s pxin_s2"></span>
							<p class="pxin_p2">月捐</p>
						</a>
					<li>

					<li class="pxin_l">
						<a href="http://www.17xs.org/ucenter/myDonate.do" class="pxin_a">
							<span class="pxin_s pxin_s3"></span>
							<p class="pxin_p3">捐助记录</p>
						</a>
					<li>
					<li class="pxin_l pxin">
						<a href="http://www.17xs.org/project/releaseH5Project.do" class="pxin_a">
							<span class="pxin_s pxin_s4"></span>
							<p class="pxin_p4">发起求助</p>
						</a>
					<li>
				</ul>

			</div>
			<c:if test="${!empty projectbannerList }">
				<div class="huzhu scroll-pic">
					<ul class="swiper-wrapper">
						<c:forEach items="${projectbannerList }" var ="pBanner">
							<li class="pic-slide">
								<a href="${pBanner.linkUrl }"><img src="${pBanner.url }" /></a>
							</li>
						</c:forEach>
					</ul>
				</div>
			</c:if>
			

			<div class="sy-special-fund">
				<h4>善园专项基金</h4>
				<div class="funds scroll-pic">
					<ul class="swiper-wrapper">
						<c:forEach items="${specialFundList }" var="specialFund">
							<li class="pic-slide">
								<a href="http://www.17xs.org/project/view/?projectId=${specialFund.id }"><img src="${specialFund.coverImageUrl }" />
									<p>${specialFund.title }</p>
								</a>
							</li>
						</c:forEach>
					</ul>
				</div>
			</div>

			<div class="sy-special-fund help">
				<h4>急难救助 <a href="http://www.17xs.org/h5/list/?extensionPeople=${extensionPeople }" class="funds-more">更多</a></h4>
				<div class="funds scroll-pic">
					<ul class="swiper-wrapper">
						<c:forEach items="${rdList }" var="project">
							<li class="pic-slide">
								<a href="http://www.17xs.org/project/view/?projectId=${project.id }"><img src="${project.coverImageUrl }" />
									<p>${project.title }</p>
								</a>
							</li>
						</c:forEach>
					</ul>
				</div>
				<hr class="hr" />
				<div class="help-list">
					<ul>
						<c:set value="${ fn:split(projectTag, ',') }" var="configs" />
						
						<c:forEach items="${configs }" var="config">
							<li>
								<a href="http://www.17xs.org/h5/list/?extensionPeople=${extensionPeople }&tag=${config }">${config}</a>
							</li>
						</c:forEach>
					</ul>
				</div>
			</div>

			<div class="sy-special-fund project-feed">
				<h4>项目动态</h4>
				<div class="feed-list">
					
				</div>
				
				<div class="loadmore"><a href="javascript:;">点击加载更多</a></div>
			</div>
		</div>
	</div>

	<div id="category" class="category">
		<div class="scroll_wrap">
			<c:if test="${size  > 6}">
				<c:forEach begin="0" end="${size}" varStatus="status" step="6">
					<ul class="categoryList clearfix">
						<c:forEach var="record" items="${atc}" begin="${status.index }" end="${status.index+5 }" varStatus="status">
							<c:if test="${record.typeName_e != 'good'}">
								<li>
									<a href="http://www.17xs.org/h5/list/?field=${status.index+1}&extensionPeople=${extensionPeople }">
										<img src="${record.coverImageUrl}">
										<span>${record.typeName}</span>
									</a>
								</li>
							</c:if>
						</c:forEach>
					</ul>
				</c:forEach>
			</c:if>
			<c:if test="${size  <= 6}">
				<ul class="categoryList clearfix">
					<c:forEach var="record" items="${atc}" varStatus="status">
						<c:if test="${record.typeName_e != 'good'}">
							<li>
								<a href="http://www.17xs.org/h5/list/?field=${status.index+1}&extensionPeople=${extensionPeople }">
									<img src="${record.coverImageUrl}">
									<span>${record.typeName}</span>
								</a>
							</li>
						</c:if>
					</c:forEach>
				</ul>
			</c:if>
		</div>
	</div>
	<div class="footer_fk">
		<div class="footer_t">
			<a href="http://www.17xs.org?flag=pc">电脑版&nbsp;|&nbsp;</a>
			<a href="http://www.17xs.org/user/userAdvice.do">用户反馈</a>
		</div>
		<div class="footer_f">
			<p>Copyright ©宁波市善园公益基金会 版权所有 浙ICP备15018913号-1</p>
		</div>
	</div>

	<div class="big-pic"><div class="scroll"></div></div>
	<div id="overLay"></div>
	<input type="hidden" id="extensionPeople" value="${extensionPeople }">
	<input type="hidden" name="cur_num" id="cur_num" value="${totalMoney}" />
	<script src="../../../../res/js/h5/iscroll.js"></script>
	<script data-main="../../../../res/js/h5/index_new.js?v=201510131033" src="../../../../res/js/require.min.js"></script>
	<script type="text/javascript" src="../../../../res/js/jquery-1.8.2.min.js"></script>
	<script src='../../../../res/js/h5/hhSwipe.js'></script>
	<script src="../../../../res/js/h5/number.js"></script>
	<script>
		$(function(){
			require(['../../../../res/js/h5/swiper.min'],function(){
				
					var per = [414/290,414/170];
					var data = [{
						slidesPerView: per[0],
						spaceBetween: 10,
						freeMode: true,
						slideClass:'pic-slide',
						slidesOffsetAfter:30
					},{
						slidesPerView: per[1],
						spaceBetween: 10,
						freeMode: true,
						slideClass:'pic-slide',
						slidesOffsetAfter:30
					}];
					new Swiper('.huzhu',data[0]);
					new Swiper('.funds',data[1]);
			});
		});
	</script>
	
	<%@ include file="cs.jsp" %>
	<%CS cs = new CS(1257726653);cs.setHttpServlet(request,response);String imgurl = cs.trackPageView();%>
</body>
</html>