		...
		init:function(){
			...
			$('#imgFile').change(function(){
				...
				if(p){
					...
					addObj.hide();
					addObj.before('<div class="item old"><img src="'+src+'" width="80px" height="80px"><i>×</i></div>');
					$('#form1').submit();
				}
				return false;
			});
...
//						}
		...
		phone:function(str){
		 ...
		 needhelpsubmit2:function(){
			    ...
			    var projectUserInfoId=$('#projectUserInfoId').val();
			    if(projectUserInfoId==""){
			    	$("#msg").html('<p>修改出现错误重新修改</p>');
					$("#msg").show();
					setTimeout(function () {
			            $("#msg").hide();
		    	...
		    	var appealAddress=$("#familyAddress").val();
		    	if(!appealName){
					$("#msg").html('<p>受助人姓名不能为空</p>');
					$("#msg").show();
					setTimeout(function () {
			            $("#msg").hide();
			        }, 2000);
				...
				$.ajax({ 
						...
						data:{id:projectId,
							...
							t:new Date().getTime(),
							linkMan:appealName,
							indetity:appealIdcard,
							linkMobile:appealPhone,
							familyAddress:appealAddress,
							projectUserInfoId:projectUserInfoId
						},
						chase:false,
						success: function(result){
							if(result.flag==1){
