...
define(["extend","dialog","head","entry","userCenter","ajaxform","pages"],function($,d,h,en,uc,f,p){
	...
	p.changeCallback=function(){
		addListNew.dataList(p.curPage,20);
	};
	addListNew={
		init:function(){
			var that=this;
					...
					if(result.flag=='0'){
						...
						if(data.total>0){
							for(var i=0;i<len;i++){
								...
								html.push('<li class="lst-col7" title="'+datas[i].realName+'">'+datas[i].realName+'</li>');

								html.push('<li class="lst-col3" title="'+datas[i].accountName+'">'+datas[i].accountName+'</li>');
								html.push('<li class="lst-col4" title="'+datas[i].account+'">'+datas[i].account+'</li>');
								html.push('<li class="lst-col2" title="'+datas[i].projectTitle+'"><a href="'+titleurl+'" target="_blank">'+datas[i].projectTitle+'</a></li>');
								html.push('<li class="lst-col5 orangeText">'+datas[i].panMoney+'</li>');
								html.push(' <li class="lst-col6"><a href="'+titleurl+'&itemType=1" title="" class="greenText" target="_blank">查看凭证</a></li>');
								html.push('</ul>');
							}
						}else{
