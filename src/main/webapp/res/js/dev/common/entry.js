...
//					window.location.href=window.baseurl+"ucenter/core/mygood.do";
					window.location.href=window.baseurl+"ucenter/core/personalCenter.do";
				}
			}).on("focus","#username",function(){ 
				var v=$(this).val();dv=$(this).attr('dValue');
		...
		spreadCode:function(obj){
			var that=this,nameObj=$(obj),nameVal = nameObj.val(),len = nameVal.cnLength(),msgObj=nameObj.parent().parent().children('.wrinfo'),primObj=nameObj.parent().parent().children('.wrsdpt');
			if(len==10){
					return true;
			}else{
				primObj.html('');
		...
		fastSign:function(obj,type){
			   var that=this,p={},name,type=type,passWord=$('#uPw').val(),code=$('#wrcode').val(),phoneCode=$('#mcode').val(),spreadCode=$('#spreadCode ').val();
		       if(type==0){
		       	name=$.trim($('#uName').val());
			   ...
			   var isRule=$("#rule").attr("checked");
  			   if(!isRule){
				  $("#signin").val("立即注册").removeAttr("disabled");
  				  d.alert({content:"对不起，请先同意我们的服务条款才可以完成注册"});
				  return false;
  			   }
			   ...
			   p.t = new Date().getTime();
				if(type==0&& spreadCode.length==10){
					var nameObj=$('#spreadCode'),primObj=nameObj.parent().parent().children('.wrsdpt'),msgObj=nameObj.parent().parent().children('.wrinfo');
					$.ajax({
					      url:WebAppUrl.spreadCode,
						  data:{code:$('#spreadCode').val(),t:new Date().getTime()}, 
						  success: function(result){ //eval(result);//回调函数  参数后台来 
							    if(result.flag==0){
							       primObj.html('');
								   ...
								   return false;
							    	...
							    	$.ajax({
										...
										error   : function(result) {
											that.rashCode(); 
											d.alert({content:'注册失败，请检查网络是否畅通。<br/>如果确认网络是通的，请联系客服人员帮助解决。',type:'error'});
											$("#signin").val("立即注册").removeAttr("disabled");
										}
									}); 
		...
		login0penUrl:function(url) {//登录->回调函数
	      ...
	      show:function(callback){ 
			  var html =[],visitor=$('#visitor').val(); 
			  html.push('<a class="closeDialogd"></a>');
	          html.push('<h2></h2>');
			  ...
			  var o ={t:"",c:html.join("")},css={width:'525px'};  
			  if(callback){this._lgCallBack=callback;} 
		  ...
		  goIn:function(obj,type){
			   ...
			   if(namev.empty() || namev.len()<4||namev.len()>32){
			         $('#warn').html('<em></em>账号填写错误，账号长度为4～32个字符。').show();
					 name.focus();
					 return;
			   } 
			   if(pwdv.empty() || pwdv.len()<6||pwdv.len()>12){
			        $('#warn').html('<em></em>密码填写错误，密码长度必须为6～12字符。').show();
					pwd.focus();
					return;
			   }
			  $('#warn').html('正在登录，请稍后...').show();
			  var di= $(obj).parents(".cp2yDialogBox").attr("data"); 
			  this.login(namev,pwdv,codev,di,type);
		  ...
		  signIn:function(isIn,name,id){
			  var that=this,html=[],head=$.cookie.get("user_head");
			  this.isIn=isIn;
			  if(isIn){  
				  if(head&&head!="good"){
					 html.push('<a class="user-icon-lt" id="personalCenter"><img src="'+decodeURIComponent(head)+'"></a><span id="hd-userName" class="hd-user">'+that.autoAddEllipsis(name,10)+'</span><span style="display: none;" id="hd-quit" class="hd-user"><a class="link-btn" id="signout">退出</a></span>');
					// html.push('<span class="onLogin"><img src="'+decodeURIComponent(head)+'" alt=""><a title="" class="blackText" id="hd-userName">'+that.autoAddEllipsis(name,10)+'</a><a title="" class="greenText" id="personalCenter">用户中心</a><span>|</span><a title="" class="blackText" id="signout">退出</a></span>')
				  } else{
						...
						//html.push('<span class="onLogin"><img src="http://www.17xs.org/res/images/user/user-icon.png" alt=""><a title="" class="blackText" id="hd-userName">'+that.autoAddEllipsis(name,10)+'</a><a title="" class="greenText" id="personalCenter">用户中心</a><span>|</span><a title="" class="blackText" id="signout">退出</a></span>')

				  }
				 $("#enter").html(html.join(''));			  
			  }else{ 
				//newhead onlogin
				// html.push('<span class="noLogin"><a href="'+WebAppUrl.HOME_URL+'user/sso.do" target="_blank" title="" class="regBtn"><em class="iconNew iconPen"></em>注册</a><a id="hd-login"  title="" class="loginBtn"><em class="iconNew iconPerson"></em>登录</a></span>')
				...
				//  html.push('<a id="qq_login_btn"></a><script type="text/javascript">QC.Login({btnId:"qq_login_btn"});</script>');
				  $("#enter").html(html.join(''));
			  } 
		...
		isFull:function(pChar) {
		 ...
		 sendcode:function(obj,phoneobj,codeobj,phonecodeboj,type){
			...
			var phone = $(phoneobj).val(),msgObj=$(obj).next('.wrsdpt'), msgerror=$(codeobj).parent().parent().children('.wrinfo'), msgerrorleg=$(phonecodeboj).parent().parent().children('.wrinfo'),types,code;
			codeobj?code=$(codeobj).val():'';
			var cause = "用户注册验证码";
			if(phone=='' || phone=='用户名/手机号码'){
				msgerror.html('<i class="error"></i>请输入正确的手机号码！');
				//d.alert({content: "请输入正确的手机号码！"});
				$(phoneobj).focus();
				return;
			} 
			if(type==0){
			...
			$.ajax( {
				...
				success : function(result) { 
						if(result.flag==1){
							...
							if(wait==60){$(obj).addClass("wrstate");that.time();}
						}else{
							 //d.alert({content: result.errorMsg});
		...
		time:function(){
			if (wait == 0) {
			   ...
			   $("#pSend").removeClass("wrstate");
				...
				wait--;
				setTimeout(function(){ entry.time(); },1000);//循环调用
				$("#pSend").html(wait + "秒后重新获取验证码");//改变对象中的内容
				//$("#pSend").addClass("sendtime");
			}
		}, 
		...
		isValidName: function(nameObj) {//nameId, promptId
		   ...
		   data={name:nameVal,t:new Date().getTime()};
		   $.ajax({
		...
		checkSureId: function (sureObj, passObj) {//checkConfirmPwd: function (confirmPassId, passId, promptId) {
			var sureObj=$(sureObj),passObj=$(passObj),msgObj=sureObj.next(),sureVal =sureObj.val(),pwdVal = passObj.val();
		...
		checkName : function(nameObj){
			nameObj = $(nameObj);
			var name=$.trim(nameObj.val()),len=name.length;
			if (len == 0) {
				return '请填写您的真实姓名';
			}
