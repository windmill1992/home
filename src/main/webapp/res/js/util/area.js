...
define(["dialog"],function(d){
	var area= {
		...
		change:function(v){
			var sels=this.selectsId,dsy=this.areaTree(),opt0=this.opt0,rate=[0];
			for(i=0;i<v;i++){ 
			...
			with(nextSel){
				...
				if(v && document.getElementById(sels[v-1]).selectedIndex>0 || !v){
					if(dsy.Exists(_rate)){
						...
						if(!!def){
							...
							if(this.firstLoad&&v==1&&def.length>=2){
				...
				if(++v<sels.length){ this.change(v); } 
			}//End with	
		},
		...
		init:function(o){
			...
			for(i=0;i<len;i++){
			  document.getElementById(sels[i]).onchange= new Function("area.change("+(i+1)+")");
			}
...
var area= {
		...
		getArea:function(){
			...
			/*
			var obj0=document.getElementById(sels[0]);
			if(obj0.options[obj.selectedIndex].text.indexOf("省份")!=-1){ 
				return false;
			}
			var obj1=document.getElementById(sels[1]);
			if(obj1.options[obj.selectedIndex].text.indexOf("地级市")!=-1){
				return false;
			} 
			var obj2=document.getElementById(sels[2]);
			if(obj2.options[obj.selectedIndex].text.indexOf("区/县级市")!=-1){
				return false;
			} 
			var address=[];
			for(i=0;i<len;i++){
				var obj=document
				.getElementById(sels[i]);
			...
			*/
			...
			return address.join(' ')+'#'+index.join(' ')+'#012';//{address:address,index:index};
