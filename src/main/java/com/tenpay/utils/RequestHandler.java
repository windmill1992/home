...
//		System.out.println("UrlEncode后 packageValue=" + packageValue);
	 ...
	 * 创建md5摘要,规则是:按参数名称a-z排序,遇到空值的参数不参加签名。
	 */
	public String createSign_new(SortedMap<String, String> packageParams) {
		StringBuffer sb = new StringBuffer();
		Set es = packageParams.entrySet();
		Iterator it = es.iterator();
		while (it.hasNext()) {
	 ...
	 * 创建md5摘要,规则是:按参数名称a-z排序,遇到空值的参数不参加签名。
	 */
	public String createSignNoKey(SortedMap<String, String> packageParams) {
		StringBuffer sb = new StringBuffer();
		Set es = packageParams.entrySet();
		Iterator it = es.iterator();
		while (it.hasNext()) {
		...
		String subSbString=sb.toString().substring(0, sb.toString().length()-1);
		String sign = MD5Util.MD5Encode(subSbString, this.charset)
				.toUpperCase();
		return sign;
